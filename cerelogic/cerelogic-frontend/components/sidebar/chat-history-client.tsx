...
export function ChatHistoryClient() {
  ...
  const fetchInitialChats = useCallback(async () => {
    setIsLoading(true)
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/chats?offset=0&limit=20`)
      if (!response.ok) {
  ...
  const fetchMoreChats = useCallback(async () => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/chats?offset=${nextOffset}&limit=20`)
      if (!response.ok) {
